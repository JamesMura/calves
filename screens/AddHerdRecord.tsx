...
export default function AddHerdRecord({
  ...
  const getDams = async () => {
    const results = (await dbContext?.database.find(
      'Select * from Cattle where (sex = ?)',
      ['Female'],
    )) as Cattle[];
  ...
  async function onSubmit(data: {
    ...
    const record = await dbContext?.database.save(
      new Cattle(
        data.tag,
        data.sex,
        data.dateOfBirth,
        ...
        <Controller
          ...
          render={({
            ...
            <View style={styles.formField}>
              {invalid && (
                <ErrorMessage>ID/Tag Number is required</ErrorMessage>
              )}
              <FloatingLabelInput
